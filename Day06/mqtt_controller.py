...
pwm = GPIO.PWM(servo_pin, 100) # 서보모터속도
...
class publisher(Thread):
    def __init__(self):
        ...
        self.host = '210.119.12.58' # 본인 PC ip적으세요
        self.port = 1883 # 회사에서는 이 포트 그대로 안씁니다.
        self.clientId = 'IOT58'
        self.count = 0
...
class subscriber(Thread):
    def __init__(self) : # 생성자
        ...
        self.host = '210.119.12.58' # Broker IP
        #self.host ='https://
        self.port = 1883
        self.clientId = 'IOT58_SUB'
        self.topic = 'pknu/monitor/control/'
        print('subscribe 스레드 시작')
        self.client = mqtt.Client(client_id=self.clientId)

    def run(self): # Thread.start() 함수를 실행하면 샐행되는 함수
